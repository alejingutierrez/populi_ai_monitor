...
const NetworkPulse: FC<Props> = ({ stats }) => {
  const cards = [
    {
      label: 'Nodos activos',
      value: fullFormatter.format(stats.nodes),
      icon: <CircleStackIcon className='h-4 w-4' />,
      delta: stats.deltas.nodesPct,
    },
    {
      label: 'Enlaces',
      value: fullFormatter.format(stats.edges),
      icon: <LinkIcon className='h-4 w-4' />,
      delta: stats.deltas.edgesPct,
    },
    {
      label: 'Densidad',
      value: `${stats.density.toFixed(1)}%`,
      icon: <ArrowsPointingOutIcon className='h-4 w-4' />,
      delta: stats.deltas.densityPct,
    },
    {
      label: 'Polarizacion',
      value: `${stats.polarization.toFixed(1)}%`,
      icon: <ShareIcon className='h-4 w-4' />,
      delta: stats.deltas.polarizationPct,
      preferLower: true,
    },
  ]
    ...
    <section className='card p-4'>
      <div className='card-header items-start gap-4 flex-col md:flex-row md:items-center'>
        <div>
          ...
          <p className='h-section'>Pulso de conectividad</p>
            ...
            >
              <div className='flex items-center justify-between gap-2'>
                <div>
                  ...
                  <p className='text-lg font-semibold text-ink'>{card.value}</p>
                </div>
