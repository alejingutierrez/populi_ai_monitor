...
const AlertTimeline: FC<Props> = ({ timeline, rules, showRules = true }) => {
  const hasData = useMemo(() => timeline.some((point) => point.total > 0), [timeline])
    ...
    <section className='card p-4'>
      <div className='card-header items-start gap-4 flex-col md:flex-row md:items-center'>
        ...
        <div className='flex items-center gap-2'>
          ...
          <button
            ...
            className='rounded-full border border-slate-200 bg-white px-3 py-1 text-[11px] font-semibold text-slate-600 hover:border-prBlue'
          >
      ...
      {collapsed ? (
        <div className='rounded-2xl border border-dashed border-slate-200 bg-slate-50 p-6 text-center text-sm text-slate-500'>
          {hasData
            ? 'Timeline colapsado. Expándelo para ver activaciones.'
            : 'No hay activaciones recientes en este rango.'}
        </div>
      ) : (
        <div className={`grid gap-4 ${showRules ? 'xl:grid-cols-[1.3fr_1fr]' : ''}`}>
          <div>
              ...
              <div className='space-y-2'>
                {rules.length ? (
                  ...
                  <div className='rounded-2xl border border-dashed border-slate-200 bg-slate-50 px-3 py-2 text-[11px] text-slate-500'>
                    No hay reglas activas en este rango.
                  </div>
                )}
              </div>
            </div>
